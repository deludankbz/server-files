...
void loadVoteFile(){
	...
	while( !pFile.EOFReached() ){
		...
		if(g_ambient_music_name.IsEmpty()){
			if(line.Find("Music: ") == 0){
				g_ambient_music_name = line.SubString( 7 );
			}
		}
...
void 
_map404_message(bool useSecondaryMap){
...
void MapInit(){
	...
	if(!g_ambient_music_name.IsEmpty())
		g_SoundSystem.PrecacheSound( g_ambient_music_name );
}
...
void music_on() {
	if(!g_ambient_music_name.IsEmpty())
	{
		dictionary keys;
		keys["origin"]	   = "0 0 0";
		keys["message"]	   = g_ambient_music_name;
		keys["health"]	   = "10";
		keys["playmode"]   = "2";
		keys["spawnflags"] = "1";
		keys["classname"]  = "ambient_generic";
...
HookReturnCode ClientSay( SayParameters@ pParams ) {
	...
	if(	args[0] == "/musicon")
	{
		CBaseEntity@ ent = g_EntityFuncs.FindEntityByClassname(ent, "ambient_generic");
		if(ent !is null)
			g_PlayerFuncs.ClientPrint(plr, HUD_PRINTTALK, "Ambient musiic is already on !");
		else
			music_on();
		pParams.ShouldHide = true;
		return HOOK_HANDLED;
	}
	else if(	args[0] == "/musicoff")
	{
		CBaseEntity@ ent = g_EntityFuncs.FindEntityByClassname(ent, "ambient_generic");
		if(ent is null)
			g_PlayerFuncs.ClientPrint(plr, HUD_PRINTTALK, "Ambient musiic entity not found !");
		else
...
void MapActivate(){
	...
	g_Scheduler.SetTimeout( "vote_logic", 1.0f );
	g_Scheduler.SetTimeout( "music_on", 1.0f );
}
